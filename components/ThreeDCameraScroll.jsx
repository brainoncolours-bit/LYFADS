...
import { Points, PointMaterial, Environment, Float, Text } from '@react-three/drei';
...
const FloatingObject = ({ position, color, speed, geometryType = 'icosahedron' }) => {
  ...
  const geometry = useMemo(() => {
    switch (geometryType) {
      case 'torus':
        return <torusGeometry args={[0.5, 0.15, 16, 100]} />;
      case 'box':
...
const CameraController = ({ scrollYProgress }) => {
  ...
  useFrame(() => {
    ...
    const x = scrollYProgress.current * Math.PI * 2;
    const y = Math.sin(scrollYProgress.current * Math.PI) * 2;
    ...
    // Smoothly look at origin with slight offset based on scroll
    camera.lookAt(
      Math.sin(scrollYProgress.current * 0.5) * 2,
      Math.cos(scrollYProgress.current * 0.3) * 1,
      0
    );
  });
...
// Floating 3D Text
const FloatingText = ({ text, position, color = "#ffffff", fontSize = 1 }) => {
  return (
...
const ThreeDScene = ({ scrollYProgress }) => {
  return (
    <>
      ...
      <FloatingText text="ABOUT" position={[0, 0, -5]} color="#ffffff" fontSize={2} />
      <FloatingText text="OUR STORY" position={[2, -2, -3]} color="#7a5af5" fontSize={1.5} />
      <FloatingText text="VISION" position={[-3, 2, -2]} color="#ff006e" fontSize={1.5} />
...
const ThreeDCameraScroll = () => {
  ...
  // Using GSAP for advanced scroll control
  useGSAP(() => {
  ...
  // Using Framer Motion for scroll detection
  const { scrollYProgress: motionScrollProgress } = useScroll({
    target: scrollRef,
    offset: ["start start", "end end"]
  });

  // Sync Framer Motion scroll progress with our custom state
  useEffect(() => {
    const unsubscribe = motionScrollProgress.on("change", (latest) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <ThreeDScene scrollYProgress={{ current: scrollYProgressValue }} />
        </Canvas>
          ...
          <div className="text-center px-4 max-w-4xl opacity-80">
            ...
            >
              ...
              <p className="text-lg md:text-xl text-gray-300 max-w-2xl mx-auto">
                Experience immersive 3D visuals that respond to your scroll
              </p>
            </motion.div>
